...
public class DownUtil {
	...
	public static void dowloadFile(File file) {
		try {
			...
			//编码的文件名字,关于中文乱码的改造
			String codeFileName = "";
			String agent = Objects.requireNonNull(ServletUtil.getRequest()).getHeader("USER-AGENT").toLowerCase();
			if (agent.contains("msie") || agent.contains("trident")) {
				//IE
				codeFileName = URLEncoder.encode(file.getName(), "UTF-8");
			} else if (agent.contains("mozilla")) {
				//火狐，谷歌
				codeFileName = new String(file.getName().getBytes(StandardCharsets.UTF_8), StandardCharsets.ISO_8859_1);
			} else {
				codeFileName = URLEncoder.encode(file.getName(), "UTF-8");
			}
	...
	public static void dowloadFile(String file) {
		try {
			...
			//读取指定路径下面的文件
			in = new FileInputStream(file);
			outputStream = new BufferedOutputStream(Objects.requireNonNull(ServletUtil.getResponse()).getOutputStream());
			//创建存放文件内容的数组
			byte[] buff = new byte[1024];
			//所读取的内容使用n来接收
			int n;

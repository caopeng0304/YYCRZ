package com.sinosoft.ie.booster.common.encrypt.util;
...
public class Util {
	...
	public static byte[] hexToByte(String hex) throws IllegalArgumentException {
		...
		/*
		 * char[] arr = hex.toCharArray(); byte[] b = new byte[hex.length() /
		 * 2]; for (int i = 0, j = 0, l = hex.length(); i < l; i++, j++) {
		 * String swap = "" + arr[i++] + arr[i]; int byteint =
		 * Integer.parseInt(swap, 16) & 0xFF; b[j] = new
	...
	public static String byteToHex(byte b[]) {
		...
		String hs = "";
		/*
		 * String stmp = ""; for (int n = 0; n < b.length; n++) { stmp =
		 * Integer.toHexString(b[n] & 0xff); if (stmp.length() == 1) { hs = hs +
		 * "0" + stmp; } else { hs = hs + stmp; } }
		 */
	 ...
	 * @return
	 */
	//private static BigInteger r = new BigInteger(256, new SecureRandom());
	 ...
	 * @return
	 */
	public static boolean isAllZero(byte[] buffer) {
	...
	public static byte[] calculateHash(BigInteger x2, byte[] M, BigInteger y2) {

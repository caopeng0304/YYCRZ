...
public class SentinelApiClient {
	...
	private static final SentinelVersion VERSION_160 = new SentinelVersion(1, 6, 0);

	private static final SentinelVersion VERSION_171 = new SentinelVersion(1, 7, 1);
	...
	protected boolean isSupportPost(String app, String ip, int port) {
		return StringUtil.isNotEmpty(app)
				&& Optional.ofNullable(appManagement.getDetailApp(app)).flatMap(e -> e.getMachine(ip, port))
						.flatMap(m -> VersionUtils.parseVersion(m.getVersion()).map(v -> v.greaterOrEqual(VERSION_160)))
						.orElse(false);
	}
	...
	protected boolean isSupportEnhancedContentType(String app, String ip, int port) {
		return StringUtil.isNotEmpty(app)
				&& Optional.ofNullable(appManagement.getDetailApp(app)).flatMap(e -> e.getMachine(ip, port))
						.flatMap(m -> VersionUtils.parseVersion(m.getVersion()).map(v -> v.greaterOrEqual(VERSION_171)))
						.orElse(false);
	}

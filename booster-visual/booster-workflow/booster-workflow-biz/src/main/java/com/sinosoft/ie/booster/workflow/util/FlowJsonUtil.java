...
public class FlowJsonUtil {
	...
	private static String nextNodeId(String data, String nodeId, List<ChildNodeList> childNodeListAll, List<ConditionList> conditionListAll) {
		...
		//条件、分流的判断
		if (StrUtil.isNotEmpty(contextType)) {
			if (FlowCondition.CONDITION.equals(contextType)) {
				...
				if (StrUtil.isNotEmpty(nextId)) {
					flag = true;
				}
			} else if (FlowCondition.INTERFLOW.equals(contextType)) {
				List<String> flowList = childNode.getFlowList().stream().map(ChildNode::getNodeId).collect(Collectors.toList());
	...
	public static void upList(List<FlowTaskNodeEntity> flowTaskNodeList, String node, Set<String> upList, String[] tepId) {
		...
		if (entity != null) {
			List<String> list = flowTaskNodeList.stream().filter(t -> t.getSort() != null && t.getSort() < entity.getSort()).map(t -> t.getNodeCode()).collect(Collectors.toList());
	 ...
	 * 节点条件判断
	 **/
	private static boolean nodeConditionDecide(String formDataJson, List<ProperCond> conditionList, Map<String, String> boosKey, Map<String, Object> keyList) {
		boolean flag = false;
		...
		for (int i = 0; i < conditionList.size(); i++) {
			...
			expression.append(formValue).append(symbol).append(filedValue);
		...
		if (childNode != null) {
			...
			if (hasconditionNodes) {
				...
				if (isFlow) {
					...
					List<String> flowIdAll = conditionNodes.stream().map(ChildNode::getNodeId).collect(Collectors.toList());
	...
	private static void getchildNode(ChildNode parentChildNode, String firstId, List<ChildNodeList> childNodeListAll, List<ConditionList> conditionListAll, List<ChildNode> chilNodeList) {
		...
		if (childNode != null) {
			...
			Custom customModel = new Custom();
			customModel.setType(childNode.getType());
			String num = chilNodeList.stream().anyMatch(t -> t.getNodeId().equals(nodeId)) ? "0" : "1";

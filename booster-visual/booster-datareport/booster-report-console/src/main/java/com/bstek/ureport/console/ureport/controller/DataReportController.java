...
//            String id = url.substring(strStartIndex + 1);
            updateData(req, resp, Convert.toLong(url));
        }
...
//            } else {
//                String id = url.substring(strStartIndex + 1);
                deleteData(req, resp, Convert.toLong(url));
//            }
        }
        if ("GET".equals(method)) {
            if (url == null) {
                getList(req, resp);
            }else if ("init".equals(url)) {
                init(req, resp);
            }else if ("Selector".equals(url)) {
                Selector(req, resp);
            }else if ("preview".equals(url)) {
                previewData(req, resp);
            }else if (url.contains("/Actions/Export/")) {
                //截取Id
                Long id = Convert.toLong(url.split("/")[0]);
                //截取type
            ...
            //if (!url.contains("/Actions/Export/") && !url.equals("DataReport/init") && !url.equals("DataReport/Selector") && !url.contains("/preview"))
                ...
                getInfo(req, resp, Convert.toLong(url));
            }
        }
    }
    ...
    public void previewData(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String token = decode(req.getHeader("Authorization"));
        if ("preview".equals(id)) {
            try {
                ...
                connection = JdbcUtil.getConnection(dataSourceUtil);
                Report report = null;
...
//                e.printStackTrace();
            ...
            //通过id预览
            ReportEntity entity = reportService.GetInfo(Convert.toLong(id));
            UreportPreviewUtil previewUtil = new UreportPreviewUtil();
            Connection connection = null;
            connection = JdbcUtil.getConnection(dataSourceUtil);
    ...
    //通过id打开到报表编辑器
    public void getInfo(HttpServletRequest req, HttpServletResponse resp, Long id) throws ServletException, IOException {
    ...
    //修改
    public void updateData(HttpServletRequest req, HttpServletResponse resp, Long id) throws ServletException, IOException {
        String payload = RequestUtil.getPayload(req);
        Map<String, Object> map = JSONUtil.StringToMap(payload);
        if (id == null) {
    ...
    //删除
    public void deleteData(HttpServletRequest req, HttpServletResponse resp, Long id) throws ServletException, IOException {
        if (id == null) {
            writeObjectToJson(resp, ActionResult.fail("数据不存在，修改失败"));
        } else {
            ReportEntity entity = reportService.GetInfo(id);
    ...
    //通过id导出报表
    public void exportData(HttpServletRequest req, HttpServletResponse resp, Long id, String type) throws ServletException, IOException {
        ReportEntity entity = reportService.GetInfo(id);
        if (entity == null) {
        ...
        if ("pdf".equals(type.toLowerCase())) {
            ...
            outputStream.flush();
            outputStream.close();
        } else if ("word".equals(type.toLowerCase())) {

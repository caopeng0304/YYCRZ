...
public class DatasourceServletAction extends RenderPageServletAction {
    ...
    public void loadBuildinDatasources(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        Connection connection = JdbcUtil.getConnection(dataSourceUtil);
        if (connection != null) {
            try {
                String dbName = "";
    ...
    public void buildDatabaseTables(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        ResultSet rs = null;
        try {
                conn = JdbcUtil.getConnection(dataSourceUtil);
            String sql = null;
            List<Map<String, String>> tables = new ArrayList<Map<String, String>>();
            if (dataSourceUtil.getDriverClassName().toLowerCase().contains(DataSourceEnum.MYSQL.getDbLinkType())) {
            ...
            } else if (dataSourceUtil.getDriverClassName().toLowerCase().contains(DataSourceEnum.ORACLE.getDbLinkType())) {
//                rs = metaData.getTables("BOOSTER", schema, "%", new String[]{"TABLE", "VIEW"});
                sql = "SELECT TABLE_NAME FROM user_tables WHERE tablespace_name = 'BOOSTER'";
            } else {
                sql = "SELECT TABLE_NAME," +
            ...
            if (dataSourceUtil.getDriverClassName().toLowerCase().contains(DataSourceEnum.ORACLE.getDbLinkType())){
                while (rs.next()) {
    ...
    private String parseSql(String sql, Map<String, Object> parameters) {
        ...
        if (sql.startsWith(ExpressionUtils.EXPR_PREFIX) && sql.endsWith(ExpressionUtils.EXPR_SUFFIX)) {
            ...
            Matcher matcher = SQL_PATTERN.matcher(sqlForUse);
            while (matcher.find()) {

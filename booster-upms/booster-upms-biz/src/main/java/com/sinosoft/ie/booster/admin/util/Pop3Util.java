package com.sinosoft.ie.booster.admin.util;
...
public class Pop3Util {
	...
	public Map<String, Object> popMail(MailAccount mailAccount) {
		List<ExtEmailReceiveEntity> entity;
	 ...
	 */
	private List<ExtEmailReceiveEntity> parseMessage(Message... messages) throws MessagingException, IOException {
		List<ExtEmailReceiveEntity> receiveEntity = new ArrayList<>();
		...
		for (Message message : messages) {
			...
			if (isContainerAttachment) {
				//邮件保存路径
				String path = fileApi.getFilePath(FileTypeEnum.MAIL);
				//保存附件
			...
			ExtEmailReceiveEntity entity = new ExtEmailReceiveEntity();
			entity.setMailAccount(getReceiveAddress(msg, null));
			entity.setMailId(getMessageId(msg));
			if (getFrom(msg) == null) {
			...
			entity.setSentDate(LocalDateTime.ofInstant(msg.getSentDate().toInstant(), ZoneId.systemDefault()));
			entity.setIsRead(0);
	...
	private String getPriority(MimeMessage msg) throws MessagingException {
		...
		if (headers != null) {
			...
			if (headerPriority.contains(StringNumber.ONE) || headerPriority.contains("High")) {
				priority = "紧急";
			} else if (headerPriority.contains(StringNumber.FIVE) || headerPriority.contains("Low")) {
				priority = "低";
			} else {
				priority = "普通";
			}
